...
while True:
    ...
    for i in range(len(pg_links)-1,-1,-1):
        ...
        try:
            ...
            action.move_to_element(driver.find_element_by_id('mbc').find_elements_by_class_name('mbcMerchantName')[0]).perform()
            time.sleep(3)
            sel_link = driver.find_element_by_class_name('mbcPopoverContainer').find_element_by_tag_name('a').get_attribute('href')
            time.sleep(1)
            print('Vendor name: ' + driver.find_element_by_id('mbc').find_elements_by_class_name('mbcMerchantName')[0].text)
...
for i in vendor_links:
    ...
    while True:
        ...
        for i in links_all:
            try:
                p = float(i.find_element_by_class_name('a-price').text.replace(' ','').replace('$','').replace('\n','.'))
#                print(p)
...
#                    print(p)
    ...
    df = pd.DataFrame(columns=["Vendor","Product Link","Price"] ,data={"Vendor":vname,"Product Link":plink,"Price":price})
